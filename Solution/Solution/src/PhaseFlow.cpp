...
std::vector<float> PhaseFlow::GetPhaseSpeedVector(const std::vector<float>& point)
{
...
void PhaseFlow::Start()
{
	...
	size_t iSize = differentialEquationOrder >= 1 ? size : 0;
	size_t jSize = differentialEquationOrder >= 2 ? size : 1;
	size_t kSize = differentialEquationOrder >= 3 ? size : 1;
	...
	for (size_t i = 0; i < iSize; ++i)
		for (size_t j = 0; j < jSize; ++j)
			for (size_t k = 0; k < kSize; ++k)
			{
				...
				if (differentialEquationOrder > xDiffOrder)
					phasePosition[xDiffOrder] = 2 * simulationRadius * (i / (float)(size - 1) - 0.5); //(float)i - (size / 2.0);

				if (differentialEquationOrder > yDiffOrder)
					phasePosition[yDiffOrder] = 2 * simulationRadius * (j / (float)(size - 1) - 0.5); //(float)j - (size / 2.0);

				if (differentialEquationOrder > zDiffOrder)
					phasePosition[zDiffOrder] = 2 * simulationRadius * (k / (float)(size - 1) - 0.5); //(float)k - (size / 2.0);
